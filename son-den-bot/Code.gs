...
// Здесь своя добыча даты, а не общая: «отбой 21.40» общая приняла бы за 21 апреля,
// а «подъём 10 сентября в 6 25» отдала бы время 10:00 вместо 06:25.
function датыИзТекста(s) {
  const сейчас = new Date();
  const сегодня = new Date(сейчас.getFullYear(), сейчас.getMonth(), сейчас.getDate());
  const найденные = [];
  let чистая = s;

  if (/позавчера/.test(чистая)) {
    const d = new Date(сегодня); d.setDate(d.getDate() - 2);
    найденные.push(d);
    чистая = чистая.replace(/позавчера/g, ' ');
  }
  if (/вчера/.test(чистая)) {
    const d = new Date(сегодня); d.setDate(d.getDate() - 1);
    найденные.push(d);
    чистая = чистая.replace(/вчера/g, ' ');
  }
  if (/сегодня/.test(чистая)) {
    найденные.push(new Date(сегодня));
    чистая = чистая.replace(/сегодня/g, ' ');
  }

  const месяцем = /(\d{1,2})\s+(январ[а-я]*|феврал[а-я]*|март[а-я]*|апрел[а-я]*|ма[йя][а-я]*|июн[а-я]*|июл[а-я]*|август[а-я]*|сентябр[а-я]*|октябр[а-я]*|ноябр[а-я]*|декабр[а-я]*)/g;
  let m;
  while ((m = месяцем.exec(s)) !== null) {
    const мес = месяцПоСлову(m[2]);
    if (мес) найденные.push(new Date(сейчас.getFullYear(), мес - 1, Number(m[1])));
  }
  чистая = чистая.replace(месяцем, ' ');

  // Дат может быть две: «9 сентября отбой, подъём 10 сентября».
  // Запись всегда идёт в день отбоя, то есть в самую раннюю.
  let дата = сегодня;
  if (найденные.length) {
    дата = найденные.reduce(function (a, b) { return a < b ? a : b; });
  }
  return { дата: дата, чистая: чистая };
}


function времяПоТексту(low) {
  const исходная = ' ' + low.replace(/ё/g, 'е') + ' ';
  const разбор = датыИзТекста(исходная);
  const дата = разбор.дата;
  const s = разбор.чистая;
